...
const loadKinh = () => {
  ...
  dataGlasses.forEach((item) => {
    kinh += `<button id='${item.id}' class='btn glasses col-4 ' > <img style='width:100%' src="${item.src}" alt=""></button>`;
  });
  return (document.getElementById("vglassesList").innerHTML = kinh);
};
loadKinh();

const chonkinh = () => {
  let kieuKinh = document.querySelectorAll(".glasses");
  kieuKinh.forEach((item) => {
    item.addEventListener("click", () => {
      let showThongTin = "";
      dataGlasses.forEach((g) => {
        if (g.id == item.id) {
          ...
          showThongTin = `${g.name} - ${g.brand} (${g.color})
                         <br>
                         <button class='btn btn-danger'>${g.price}</button></button> <span class='text-success'>Stocking</span>
                         <br>
                         <br>
                         ${g.description} 
                         `;
